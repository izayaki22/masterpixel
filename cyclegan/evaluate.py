...
class FIDDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.original_domain:
            ...
            return self.translator(img.to(self.device)).detach()

def get_translator(ckpt_path):
    # cycleGAN translation
    input_shape = (3, 256, 256)  
    G_BA = GeneratorResNet(input_shape, 9)  
    G_BA.load_state_dict(torch.load(ckpt_path))
    G_BA.eval()
    return G_BA
...
if __name__ == '__main__':
    ...
    root = "/root/MAIN/result/cycleGAN_200epoch"
    model_names = [name for name in os.listdir(os.path.join(root, 'saved_models')) if 'G_BA' in name]
