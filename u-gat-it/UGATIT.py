...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.trainA = ImageFolder(os.path.join('/root/MAIN/data', self.dataset, 'trainA'), train_transform)
        self.trainB = ImageFolder(os.path.join('/root/MAIN/data', self.dataset, 'trainB'), train_transform)
        self.testA = ImageFolder(os.path.join('/root/MAIN/data', self.dataset, 'testA'), test_transform)
        self.testB = ImageFolder(os.path.join('/root/MAIN/data', self.dataset, 'testB'), test_transform)
    ...
    def train(self):
        ...
        start_iter = 1
        if self.resume:
            model_list = glob(os.path.join(self.result_dir, self.experiment_name, 'saved_models', '*.pt'))
            if not len(model_list) == 0:
                model_list.sort()
                start_iter = int(model_list[-1].split('_')[-1].split('.')[0])
                self.load(os.path.join(self.result_dir, self.experiment_name, 'saved_models'), start_iter)
                print(" [*] Load SUCCESS")
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                ...
                cv2.imwrite(os.path.join(self.result_dir, self.experiment_name, 'images', 'A2B_%07d.png' % step), A2B * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.experiment_name, 'images', 'B2A_%07d.png' % step), B2A * 255.0)
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()
            ...
            if step % self.save_freq == 0:
                ...
                torch.save(params, os.path.join(self.result_dir, self.experiment_name, "saved_models", 'params_%07d.pt' % step))
            ...
            if step % 1000 == 0:
                ...
                params['disLA'] = self.disLA.state_dict()
                params['disLB'] = self.disLB.state_dict()
                torch.save(params, os.path.join(self.result_dir, self.experiment_name, "saved_models", 'params_latest.pt'))
