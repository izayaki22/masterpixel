import torch
from models import Generator, ResidualBlock, Encoder
...
class FIDDataset(torch.utils.data.Dataset):
    def __init__(self, files, device, translator, transform=None, original_domain=True):
        ...
        self.encoder = translator[0]
        self.generator = translator[1]
    ...
    def __getitem__(self, i):
        ...
        if self.original_domain:
            ...
            _, z = self.encoder(img.to(self.device))
            image = self.generator(z).detach()
            return image

def get_translator(ckpt_path, device):
    # UNIT translation

    shared_dim = 64 * 2 ** 2
    shared_E = ResidualBlock(features=shared_dim)
    shared_G = ResidualBlock(features=shared_dim)
    E_2 = Encoder(dim=64, n_downsample=2, shared_block=shared_E)
    G_1 = Generator(dim=64, n_upsample=2, shared_block=shared_G)
    E_2.load_state_dict(torch.load(ckpt_path[0]))
    G_1.load_state_dict(torch.load(ckpt_path[1]))
    E_2.eval()
    G_1.eval()

    return (E_2.to(device), G_1.to(device))
...
def calculate_fid_given_paths(paths, batch_size, ckpt_path, device, dims, num_workers=1):
    ...
    translator = get_translator(ckpt_path, device)
...
if __name__ == '__main__':
    sys.path.append('/root/MAIN/inception.py')

    root = "/root/MAIN/result/UNIT_200epoch_again"
    model_names = [name for name in os.listdir(os.path.join(root, 'saved_models')) if 'G1' in name]
    saved_epochs = [int(name.split('_')[-1].split('.')[0]) for name in model_names]
    saved_epochs.sort()
    ...
    for epoch in saved_epochs:
        encoder_name = f'E2_{epoch}.pth'
        generator_name = f'G1_{epoch}.pth'
        print(f"evaluating checkpoint {generator_name}")
        fid_value = calculate_fid_given_paths(paths=paths,
                                          batch_size=batch_size,
                                          device=device,
                                          ckpt_path= (os.path.join(root, 'saved_models', encoder_name), (os.path.join(root, 'saved_models', generator_name))),
                                          dims=dims,
                                          num_workers=num_workers)
        print("FID:", fid_value)
