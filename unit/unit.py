...
class UNIT():
    ...
    def __train__(self):
        ...
        if opt.epoch != 0:
            ...
            E1.load_state_dict(torch.load("result/%s/saved_models/E1_%d.pth" % (opt.experiment_name, opt.epoch)))
            E2.load_state_dict(torch.load("result/%s/saved_models/E2_%d.pth" % (opt.experiment_name, opt.epoch)))
            G1.load_state_dict(torch.load("result/%s/saved_models/G1_%d.pth" % (opt.experiment_name, opt.epoch)))
            G2.load_state_dict(torch.load("result/%s/saved_models/G2_%d.pth" % (opt.experiment_name, opt.epoch)))
            D1.load_state_dict(torch.load("result/%s/saved_models/D1_%d.pth" % (opt.experiment_name, opt.epoch)))
            D2.load_state_dict(torch.load("result/%s/saved_models/D2_%d.pth" % (opt.experiment_name, opt.epoch)))
        else:
            # Initialize weights
        ...
        dataloader = DataLoader(
            ImageDataset("/root/MAIN/data/%s" % opt.dataset_name, transforms_=transforms_, unaligned=True),
        ...
        val_dataloader = DataLoader(
            ImageDataset("/root/MAIN/data/%s" % opt.dataset_name, transforms_=transforms_, unaligned=True, mode="test"),
            batch_size=5,
            shuffle=True,
            num_workers=1,
        )
        ...
        def sample_images(batches_done):
            ...
            save_image(img_sample, "result/%s/images/%s.png" % (opt.experiment_name, batches_done), nrow=5, normalize=True)
        ...
        for epoch in range(opt.epoch, opt.n_epochs):
            ...
            if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
                ...
                torch.save(E1.state_dict(), "result/%s/saved_models/E1_%d.pth" % (opt.experiment_name, epoch))
                torch.save(E2.state_dict(), "result/%s/saved_models/E2_%d.pth" % (opt.experiment_name, epoch))
                torch.save(G1.state_dict(), "result/%s/saved_models/G1_%d.pth" % (opt.experiment_name, epoch))
                torch.save(G2.state_dict(), "result/%s/saved_models/G2_%d.pth" % (opt.experiment_name, epoch))
                torch.save(D1.state_dict(), "result/%s/saved_models/D1_%d.pth" % (opt.experiment_name, epoch))
                torch.save(D2.state_dict(), "result/%s/saved_models/D2_%d.pth" % (opt.experiment_name, epoch))
